		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	strcpy(releasebuf,config->common->inquiry_string+24);
	snprintf(config->common->inquiry_string, sizeof config->common->inquiry_string,
		 "%-8s%-16s%s", config->common->vendor_string,
		 config->common->product_string, releasebuf);
	return size;
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	strcpy(releasebuf,config->common->inquiry_string+24);
	snprintf(config->common->inquiry_string, sizeof config->common->inquiry_string,
		 "%-8s%-16s%04x", config->common->vendor_string,
		 config->common->product_string, releasebuf);
	return size;
}
			    ...
			    const char *buff, size_t size)
{
	...
	if(cdev == NULL){
		pr_err("android_usb:%s, not bind to controller",enabled?"enable":"disable");
		return 0;
	}
...
#if defined(CONFIG_DEFERRED_MODULE_INIT) && defined(CONFIG_USB_G_ANDROID)
deferred_late_init(init);
#else
late_initcall(init);
#endif
